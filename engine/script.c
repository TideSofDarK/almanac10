...
#define api_integer_getter(n, t, f)        \
...
#define api_integer_setter(n, t, f)              \
    static int n(lua_State *L)                   \
    {                                            \
        lua_getfield(L, 1, "__self");            \
        void **bp = lua_touserdata(L, -1);       \
...
#define api_vector_getter(n, t, f)         \
    static int n(lua_State *L)             \
    {                                      \
        lua_getfield(L, 1, "__self");      \
        void **bp = lua_touserdata(L, -1); \
...
#define api_vector_setter(n, t, f)         \
    static int n(lua_State *L)             \
    {                                      \
        lua_getfield(L, 1, "__self");      \
        void **bp = lua_touserdata(L, -1); \
                                                                ...
                                                                api_methods_getter_setter(creature_, attack_dice)
                                                                    api_methods_getter_setter(creature_, attack_dice_count)
                                                                        api_methods_getter_setter(creature_, attack_bonus)
                                                                            api_methods_getter_setter(creature_, movement_capability){0, 0}};
...
static inline void require_api(lua_State *L)
{
    ...
    /* API: Creature */
    lua_pushinteger(L, MOVCAP_NONE);
    lua_setglobal(L, "MOVCAP_NONE");
    lua_pushinteger(L, MOVCAP_GROUND);
    lua_setglobal(L, "MOVCAP_GROUND");
    lua_pushinteger(L, MOVCAP_FLY);
    lua_setglobal(L, "MOVCAP_FLY");

    lua_pushinteger(L, AISTATE_NONE);
    lua_setglobal(L, "AISTATE_NONE");
    lua_pushinteger(L, AISTATE_IDLE);
    lua_setglobal(L, "AISTATE_IDLE");
    lua_pushinteger(L, AISTATE_ROAM);
    lua_setglobal(L, "AISTATE_ROAM");
    lua_pushinteger(L, AISTATE_FLEE);
    lua_setglobal(L, "AISTATE_FLEE");
    lua_pushinteger(L, AISTATE_ATTACKING);
    lua_setglobal(L, "AISTATE_ATTACKING");
    lua_pushinteger(L, AISTATE_CHASING);
    lua_setglobal(L, "AISTATE_CHASING");
